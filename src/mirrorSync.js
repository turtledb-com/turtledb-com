...
export async function mirrorSync (name, turtleDB, signer, folder = '.') {
  ...
  const log = (action, path) => {
    ...
    logDebug(() => console.log(`(mirrorSync) ${action} ${folder} ("${name}") / ${relativePath}`))
  }
  ...
  // turtleBranch.recaller.watch(`mirrorSync"${publicKey}"`, async () => {
  //   skipCommit = true
  ...
  //                     const namePath = join(folder, name)
  //                     try {
  ...
  //       }
  //     }
  //     if (changes.length) log(`${changes.length} changes`, folder)
